...
func Test_HTTPSerchSpyFamilyCharacter(t *testing.T) {
	...
	t.Run("No name provided", func(t *testing.T) {
		...
		const EXPECTED_BODY = "Domain is empty, please provide the domain u want to challenge."

		httpRecorder := httptest.NewRecorder()
		ginCtx, _ := gin.CreateTestContext(httpRecorder)

		var err error
		ginCtx.Request, err = http.NewRequest("GET", "/HttpChallenge", nil)
		if err != nil {
			t.Errorf("Failed to create request: %s", err)
			return
		}

		server.OracleHTTPChallenge(ginCtx)
